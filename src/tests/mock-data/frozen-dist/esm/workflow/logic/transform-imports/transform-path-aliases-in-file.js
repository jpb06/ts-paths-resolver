...
export const transformPathAliasesInFile = (args) => (pathsAliases) => pipe(Effect.gen(function* () {
    const fileContent = yield* readFileAsStringEffect(`${args.distPath}/${args.sourceFilePath}`);
    const isWildcardPath = pathsAliases[1][0].endsWith('/*');
    if (isWildcardPath) {
        return yield* transformWildcardAliases(args, pathsAliases, fileContent);
    }
